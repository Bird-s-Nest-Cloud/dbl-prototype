...
import { Plus, Search, ChevronDown, X, Briefcase, Layers } from "lucide-react";

export default function DesignationSettings() {
  const [isModalOpen, setIsModalOpen] = useState(false);

  const designations = [
    { title: "Project Manager", department: "Operations", status: "Active" },
    { title: "Site Engineer", department: "Engineering", status: "Active" },
    { title: "Accountant", department: "Finance", status: "Active" },
    { title: "HR Officer", department: "HR", status: "Active" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Designation Settings</h1>
          <p className="text-slate-500 text-sm mt-1">Manage employee designations.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900 text-white">
                <th className="py-4 px-6 text-sm font-medium">Designation Title</th>
                <th className="py-4 px-6 text-sm font-medium">Department</th>
            ...
            <tbody>
              {designations.map((item, index) => (
                ...
                >
                  ...
                  <td className="py-4 px-6 text-sm text-slate-500">{item.department}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col">
            ...
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-slate-900">Add New Designation</h2>
                <p className="text-slate-500 text-sm mt-0.5">Define a new employee role title.</p>
              </div>
              <button 
